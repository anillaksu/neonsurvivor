...
class NeonSurvivor {
    ...
    setupJoystickEvents() {
     ...
     gameOver() {
         ...
         // Remove the separate mobile restart button - we'll handle restart in the canvas overlay
     }
     
     showMobileRestartButton() {
         // This method is no longer needed - we'll handle restart through canvas clicks
     ...
     restart() {
         ...
         // No need to hide restart button since we're not using it anymore
     }
     ...
     drawLevelUpBanner() {
         ...
         const titleSize = Math.max(24, Math.min(48, baseSize * 0.08));
         const textSize = Math.max(16, Math.min(24, baseSize * 0.04));
     ...
     drawUpgradeMenu() {
         ...
         // Calculate responsive font sizes
         const baseSize = Math.min(this.canvas.width, this.canvas.height);
         const titleSize = Math.max(20, Math.min(32, baseSize * 0.06));
         const fontSize = Math.max(14, Math.min(20, baseSize * 0.035));
         const descSize = Math.max(12, Math.min(18, baseSize * 0.03));
         ...
         this.ctx.shadowColor = '#00ffff';
         this.ctx.shadowBlur = 10;
         this.ctx.fillText('CHOOSE UPGRADE', this.canvas.width / 2, baseSize * 0.15);
         
         this.ctx.font = `${fontSize}px monospace`;
         
         for (let i = 0; i < this.currentUpgradeOptions.length; i++) {
             const option = this.currentUpgradeOptions[i];
             const y = (baseSize * 0.25) + i * (baseSize * 0.15);
             const margin = Math.max(20, baseSize * 0.05);
             const boxHeight = Math.max(50, baseSize * 0.12);
             
             this.ctx.fillStyle = 'rgba(0, 255, 255, 0.1)';
             this.ctx.fillRect(margin, y - boxHeight/2, this.canvas.width - margin * 2, boxHeight);
             
             this.ctx.strokeStyle = '#00ffff';
             this.ctx.lineWidth = 2;
             this.ctx.strokeRect(margin, y - boxHeight/2, this.canvas.width - margin * 2, boxHeight);
             
             this.ctx.fillStyle = '#00ffff';
             this.ctx.textAlign = 'left';
             this.ctx.fillText(`${i + 1}. ${option.name}`, margin + 10, y - boxHeight/4);
             
             this.ctx.font = `${descSize}px monospace`;
             this.ctx.fillStyle = '#aaaaaa';
             this.ctx.fillText(option.description, margin + 10, y + boxHeight/4);
             this.ctx.font = `${fontSize}px monospace`;
         }
         ...
         this.ctx.font = `${descSize}px monospace`;
         this.ctx.textAlign = 'center';
         const instructionText = 'Tap 1, 2, or 3';
         this.ctx.fillText(instructionText, this.canvas.width / 2, this.canvas.height - baseSize * 0.05);
     ...
     drawGameOver() {
         ...
         this.ctx.fillText('GAME OVER', centerX, centerY - titleSize);
         
         // Game stats
         this.ctx.font = `${textSize}px monospace`;
         this.ctx.fillText(`Final Score: ${this.score}`, centerX, centerY - textSize/2);
         this.ctx.fillText(`Level Reached: ${this.level}`, centerX, centerY + textSize/2);
         ...
         this.ctx.fillText(restartText, centerX, centerY + titleSize);
